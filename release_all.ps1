...
.DESCRIPTION
    ...
    9. Writes and signs the update manifest, then deploys the website.
    10. Mirrors a backup to F:\.
...
if (Test-Path "G:\My Drive") {
    Write-Host "`n==> Step 4: Syncing to Google Drive ($gdriveTarget)..." -ForegroundColor Cyan
    New-Item -ItemType Directory -Force -Path $gdriveTarget | Out-Null

    # The two that are wanted on Drive, and only those. It used to also copy
    # the web bundle, three legacy-named duplicates, and unpacked copies of
    # both builds - into a folder that then had to sync all of it.
    ...
    Write-Host "  [OK] Google Drive updated (Windows, Android)." -ForegroundColor Green
}

# ---------------------------------------------------------------------------
# 5. Git Commit & Push
# ---------------------------------------------------------------------------
Write-Host "`n==> Step 5: Committing & Pushing to GitHub..." -ForegroundColor Cyan
git add .
...
if (-not $SkipCloudBuild) {
    Write-Host "`n==> Step 6: Waiting for the macOS & Linux Cloud Build..." -ForegroundColor Cyan
    ...
    Write-Host "`n==> Step 7: Downloading native macOS & Linux binaries from cloud..." -ForegroundColor Cyan
...
# ---------------------------------------------------------------------------
Write-Host "`n==> Step 8: Publishing to GitHub Releases ($releaseTag)..." -ForegroundColor Cyan
# One name per artifact. The cloud build used to produce a second, legacy
# spelling of each desktop file and both were attached - nine assets for four
# platforms. These are the names the website and the update manifest link to.
$releaseAssets = @(
...
# killed the pipeline on every brand-new version tag, after the push had
# already happened. Probe with the preference relaxed and read the
...
if ($releaseAlreadyExists) {
    ...
    gh release create $releaseTag $releaseAssets --title "SubTask Manager $releaseTag" --notes "Automated release build supporting Windows, Android, macOS, Linux and Steam Deck."
}
...
Write-Host "`n==> Step 8b: Writing update manifest..." -ForegroundColor Cyan
...
if ($macDmg) { Add-Download $downloads 'macos' $macDmg.FullName "SubTaskManager-macOS.dmg" }
if ($linuxTar) { Add-Download $downloads 'linux' $linuxTar.FullName "SubTaskManager-Linux-x64.tar.gz" }
...
# ---------------------------------------------------------------------------
if (-not $SkipDeploy) {
    Write-Host "`n==> Step 9: Deploying Website to Cloudflare Pages (subtaskmanager.com)..." -ForegroundColor Cyan
...
if (Test-Path ".\backup_to_f.ps1") {
    Write-Host "`n==> Step 10: Mirroring full backup to F:\subTask Manager..." -ForegroundColor Cyan
